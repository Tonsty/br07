...
struct opts_t {
  ...
  bool use_global_points; // read points from a global_points.txt style file instead of directoy from corr_*
  bool suppress_optimization; // don't do spring optimization
  ...
  char *camera_centers_file; // list of camera centers
  char *correspondences_file; // file to write out correspondences to

  float max_allowed_err, min_allowed_stability, min_allowed_max_stability, max_allowed_divergence;
  float min_target_dist2;

  float lambda; // for the tps
  ...
  opts_t(int argc, char **argv) : write_individual_points(false), use_global_points(false),
    ...
    for (int i = 1; i < argc; i++) {
      ...
      } else if (!strcmp(argv[i], "-no_optimization")) {
          suppress_optimization = true;
          fprintf(stderr, "Spring optimization disabled.\n");
      } else if (!strcmp(argv[i], "-cc")) {
        // List of camera centers to do conformance smoothing
        // (i.e. instead of initializing points to the average
        // position of all correspondences, use the ray intersection
        assert(i < (argc - 1));
        camera_centers_file = argv[++i];
        fprintf(stderr, "Using camera centers from file \"%s\".\n", camera_centers_file);
      } else if (!strcmp(argv[i], "-wc")) {
        // Write correspondences to specified file
        assert(i < (argc - 1));
        correspondences_file = argv[++i];
      ...
      } else if (!strcmp(argv[i], "-global_points")) {
        // global points file is specified on stdin
...
class threaded_alignment {
  ...
  struct align_scan_struct {
	...
	align_scan_struct (const opts_t &_opts = opts_t(0, NULL),
		const vector<char *> &_mesh_names = vector<char *>(),
		const vector<corr_vector> &_corrs = vector<corr_vector>(),
		const vector<point> &_targets = vector<point>(),
		const vector<bool> &_use_points = vector<bool>(),
		const vector<float> &_confidence = vector<float>(),
		TriMesh *_points_mesh = NULL,
		const vector<TriMesh::Face> &_all_faces = vector<TriMesh::Face>(),
		threaded_alignment *_ta = NULL): 
				opts(_opts), 
				mesh_names(_mesh_names), 
				corrs(_corrs), 
				targets(_targets), 
				use_points(_use_points), 
				confidence(_confidence), 
				points_mesh(_points_mesh),
				all_faces(_all_faces),
				ta(_ta){}
  } data;
 ...
 public:
  threaded_alignment(const opts_t &opts, const vector<char *> &mesh_names,
    ...
    align_scan_struct d = align_scan_struct( opts, mesh_names, corrs, targets, use_points,
