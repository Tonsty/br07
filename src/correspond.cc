...
int main(int argc, char *argv[]) {
  ...
  for (int src_num = 0; src_num < (int) mesh_names.size(); src_num++) {
    ...
    if (opts.bbox_list) {
      ...
      if (tgt_mesh->bbox.max[0] < bbox[0] - 2 * opts.bbox_dist) skip_mesh = true;
      if (tgt_mesh->bbox.max[1] < bbox[1] - 2 * opts.bbox_dist) skip_mesh = true;
      if (tgt_mesh->bbox.max[2] < bbox[2] - 2 * opts.bbox_dist) skip_mesh = true;
      if (tgt_mesh->bbox.min[0] > bbox[3] + 2 * opts.bbox_dist) skip_mesh = true;
      if (tgt_mesh->bbox.min[1] > bbox[4] + 2 * opts.bbox_dist) skip_mesh = true;
      if (tgt_mesh->bbox.min[2] > bbox[5] + 2 * opts.bbox_dist) skip_mesh = true;
    }
